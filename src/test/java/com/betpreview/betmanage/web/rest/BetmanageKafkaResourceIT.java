...
import com.betpreview.betmanage.config.KafkaProperties;
...
class BetmanageKafkaResourceIT {
    ...
    void setup() {
        ...
        BetmanageKafkaResource kafkaResource = new BetmanageKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/betmanage-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/betmanage-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
